...
def detectObject(net, total_layer_names, image_height, image_width, image, name_colors, class_labels,indexno,  
            Boundingboxes=None, confidence_value=None, class_ids=None, ids=None, detect=True):
#def detectObject(net, height, width, image, classes, name_colors, class_labels, indexno,total_layer_names,image_height,image_width,detect=True):
    name_colors = (0, 255, 0)  # Green color for bounding boxes (modify as needed)
    indexno = 0                # Index for tracking objects (modify logic if needed)

    if detect:
        blob_object = cv.dnn.blobFromImage(image,1/255.0,(416, 416),swapRB=True,crop=False)
        net.setInput(blob_object)
        cnn_outs_layer = net.forward(total_layer_names)
